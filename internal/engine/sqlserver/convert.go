...
func (c *cc) convertAlter_table_stmtContext(n *parser.Alter_tableContext) ast.Node {
	if n.ADD() != nil {
		def := n.Column_def_table_constraints()
		stmt := &ast.AlterTableStmt{
			Table: parseTableName(n.Table_name(1)),
			Cmds:  &ast.List{},
		}

		for _, idef := range def.AllColumn_def_table_constraint() {
			col_def := idef.Column_definition()
			col_name := col_def.Id_().GetText()
			col_type := col_def.Data_type().GetText()
			col_is_not_null := hasNotNullConstraint(col_def)
